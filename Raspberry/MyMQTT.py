...
class MQTT():
    def __init__(self, broker, clientID, port, notifier):
        ...
        self._paho_mqtt.username_pw_set("KDEZIRg5NyMxMB4bDx8dEQo", "onBDYhjuoIz7Car8+npMStSa")
    ...
    def publish(self, topic, msg):
        if 'channels' in topic: 
                ...
                for el in msg["e"]:
                        if el["n"] == "temperature":
                                data = el["v"]
                                field1 = "field1="+str(data)
                        elif el["n"] == "humidity":
                                data = el["v"]
                                field2 = "field2="+str(data)
                        elif el["n"] == "CO":
                                data = el["v"]
                                field3 = "field3="+str(data)
                message = field1+"&"+field2+"&"+field3
                self._paho_mqtt.publish(topic, message, 0, False)
        self._paho_mqtt.publish(topic, json.dumps(msg), 0, False)
